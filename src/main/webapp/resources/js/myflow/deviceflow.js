(function($) {
	...
	myflow.init = function(c, o) {
		...
		if (myflow.config.editable) {
			// 工具栏
			$("#deviceflow_tools").draggable({
						handle : '#deviceflow_tools_handle'
					}).css(myflow.config.tools.attr);

			$('#deviceflow_tools .node').hover(function() {
						$(this).addClass('mover');
					}, function() {
						$(this).removeClass('mover');
					});
			$('#deviceflow_tools .selectable').click(function() {
			...
			$('#deviceflow_tools .state').each(function() {
						$(this).draggable({
									helper : 'clone'
								});
					});
